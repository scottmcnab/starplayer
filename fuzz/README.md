...
`cargo-fuzz` targets for the loaders, plus the seed corpus they start from. This is
M2-task-C7, extended by M6-task-G1 with the IT pair; the invariant is the one in those task
files and in `plans/product/02-roadmap.md` T7: **a loader never panics, never OOMs, and
always returns either `Err` or a clamped-but-valid `Module`.**
...
All of them need a **nightly** toolchain and `cargo install cargo-fuzz`: libFuzzer needs
...
synthesised MOD and MTM, the small hand-built edge cases from C3b and C5, and G1's eight
synthesised ITs) and the repository owner's own S3Ms, which already live in
`crates/starplayer-s3m/tests/fixtures/`.
