...
//! # A copy, and why
//!
//! This is `starplayer_host::transport` (M3-D4) moved across unchanged. That module is
//! already `std`-free in shape — `AtEnd`, a [`GainRamp`] and `core::mem::take` are all it
//! touches — but it lives in a `std` crate, and a crate in
//! [`NO_STD_CRATES`](../../../xtask/src/main.rs) may not depend on one. Sharing it would
//! take a `std` feature split of `starplayer-host`, which M8's master plan puts out of
//! scope ("a `no_std` port of `starplayer-host` itself — I1 records where its cadence
//! logic could later be shared, and stops there"). So it is duplicated here, deliberately
//! and with the duplication recorded: see this crate's module documentation.
//!
