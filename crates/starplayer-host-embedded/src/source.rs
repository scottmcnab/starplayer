//! The playback source this host installs, and the mailbox it reads seeks from.
//!
//! # A copy, and what had to change
//!
//! This is `starplayer_host::source` (M3-D4) with two substitutions, and they are the
//! reason it could not simply be re-exported from there (research point 1):
//!
//! * `std::sync::Arc` becomes [`starplayer_rt::Arc`] — `alloc::sync::Arc` does not exist
//!   on a target without compare-and-swap, which is exactly the bare-metal target CI
//!   checks;
//! * `core::sync::atomic::{AtomicU8, AtomicU64}` become [`starplayer_rt::atomic`]'s
//!   `AtomicU8` and a pair of [`SeqlockU64`]s, for the same reason:
//!   `riscv32imc-unknown-none-elf` implements no atomics at all, and even with
//!   `portable-atomic` in front of it there is no 64-bit one to be had without taking a
//!   lock — see [`crate::seqlock`].
//!
//! So the module is `std`-free in *shape* but not in *text*, and lifting it into a shared
//! `no_std` module of `starplayer-host` would take a `std` feature split of that crate —
//! out of scope for M8. The duplication is recorded in this crate's module documentation.
//!
//! # Why the mailbox is atomic
//!
//! The two halves of an [`EmbeddedPlayer`](crate::EmbeddedPlayer) are meant to be moved
//! apart: on the ESP32-A1S the render half lives in the I2S DMA refill and the control
//! half in an Embassy task, and on the dual-core ESP32 those are not the same core. Three
//! relaxed atomics with one release/acquire edge on the discriminant carry a seek across
//! that boundary with no lock and no allocation.
...
use starplayer_rt::atomic::{AtomicU8, Ordering};
...
/// a person holding a skip button means. Writes are ordered so the discriminant is
/// published last and read first, so a reader never sees a kind whose argument has not
/// arrived.
#[derive(Debug, Default)]
pub struct SeekMailbox {
    kind: AtomicU8,
    /// The order, row or song frame `kind` names. A [`SeqlockU64`] rather than an
    /// `AtomicU64` because the bare-metal target has no 64-bit atomic; see that type.
    argument: SeqlockU64,
    frame: SeqlockU64,
}
...
impl SeekMailbox {
    ...
    pub fn request(&self, request: SeekRequest) {
        ...
        self.argument.store(argument);
        self.frame.store(request.frame.0);
    ...
    pub fn peek(&self) -> SeekRequest {
        ...
        let argument = self.argument.load();
        let frame = Frame(self.frame.load());
        let kind = match kind {
...
/// per-module would leave the audio side the last owner of the outgoing pair on every
/// load, and dropping an `Arc` there is a `free()` inside the DMA refill.
#[derive(Clone, Debug)]
...
pub fn scan_module(module: &Arc<Module>, sample_rate_hz: u32) -> Result<ScannedSong, Error> {
    starplayer::scan_song(module, sample_rate_hz, ScanLimits::for_rate(sample_rate_hz))
}

/// Build the playback source, scanning the song first.
///
...
pub fn build_source(module: Arc<Module>, sample_rate_hz: u32, start: SeekKind, frame: Frame, handles: &SourceHandles) -> Result<BuiltSource, Error> {
    let scanned = scan_module(&module, sample_rate_hz)?;
...
mod tests {
    ...
    fn the_latest_seek_replaces_one_that_has_not_been_consumed() {
        ...
        assert_eq!(mailbox.peek(), SeekRequest { kind: SeekKind::Frame(500), frame: Frame(20) }, "the newest wins");
    }
