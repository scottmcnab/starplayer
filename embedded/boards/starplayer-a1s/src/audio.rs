...
//! The refill is an Embassy task that awaits the DMA's available-space future, renders one
//! complete [`DESCRIPTOR_BYTES`] blocks into static scratch, and copies them through a
...
//! three descriptors. The ring is therefore nine quanta = 4 608 bytes: the smallest
//! whole-quantum multiple of three at least as deep as the old ring, producing three equal
//! 1 536-byte descriptors. Each descriptor is exactly three render quanta, 384 stereo frames,
//! or about 8.7 ms.
//!
//! Hardware established why that exact geometry and `push` discipline matter. With the old
...
//! 89–90 KB/s with about 43 pushes/s — roughly half the required 176 400 B/s. Equal descriptors
//! and full closure consumption keep the write offset and descriptor ownership aligned. See
...
//! 2. **Ring depth.** [`DMA_RING_QUANTA`] is 9 — 1 152 frames, 26.1 ms — because three equal
//!    descriptors must each contain a whole number of render quanta. Research point 4 still asks
//!    for the smallest geometry that never underruns with the radio off. The control task prints
//!    `underruns=` once a second; preserve the three-descriptor, whole-quantum constraints in any
//!    later sweep.
...
const _: () = assert!(DESCRIPTOR_BYTES == 1_536, "the board-proven descriptor is 1 536 bytes");
const _: () = assert!(DMA_RING_BYTES == 4_608, "the board-proven ring is 4 608 bytes");
...
///
/// A task local `[i16; 768]` would enlarge the task future and could materialise on the spawning
...
/// 1.1.2. StarPlayer's descriptors are 384 frames, or 8.7 ms at 44.1 kHz, so this is a
/// bounded roughly 70 ms pre-roll. It changes only muted startup time, not the steady-state
...
/// playing whatever is in the buffer the moment it starts, and 26 ms of silence at the top
/// of a song is 26 ms of a click waiting to happen when the codec unmutes. Rendering the
...
pub fn start(parts: Parts, sample_rate_hz: u32, render: &mut RenderHalf<Linear>) -> Result<AudioTransfer, Error> {
    ...
    // The macro chunk controls allocation count: 4 608 / 1 536 is exactly three. esp-hal's
...
/// written.
///
/// `destination` is the whole nine-quantum DMA ring during the synchronous prefill. The helper
/// retains its tail handling so `RenderHalf::render` keeps stream alignment if this caller ever
/// changes; steady state uses [`render_descriptor`] and never renders a partial descriptor.
...
/// 89–90 KB/s and about 43 pushes/s. Stereo 16-bit at 44.1 kHz requires 176 400 B/s. The ragged
...
/// state: the closure loops over every complete 1 536-byte descriptor in its contiguous offer
        ...
        if transfer
            .push_with(|destination| {
                ...
                for descriptor in destination.chunks_exact_mut(DESCRIPTOR_BYTES) {
                    ...
                    descriptor.copy_from_slice(bytemuck::cast_slice(&descriptor_scratch[..]));
