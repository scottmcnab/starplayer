...
readonly EXPECTED_IMAGE="${TEST_EMBEDDED_DIR}/target/starplayer-a1s-web-merged.bin"
readonly WRONG_IMAGE="${TEST_ROOT}/other checkout/embedded/target/starplayer-a1s-web-merged.bin"
...
if [[ "$#" -eq 3 && "$1" == clean && "$2" == -p && "$3" == starplayer-embedded-xtask ]]; then
    ...
    printf '%s\n' cargo-clean >> "${EVENTS}"
elif [[ "$#" -eq 7 && "$1" == xtask && "$2" == image && "$3" == --board && "$4" == a1s && "$5" == --features && "$6" == web && "$7" == --merge ]]; then
    printf '%s\n' cargo-image >> "${EVENTS}"
    if [[ "${FAKE_BUILD_MODE}" == local ]]; then
        mkdir -p -- "$(dirname -- "${EXPECTED_IMAGE}")"
        printf '%s\n' fresh-image > "${EXPECTED_IMAGE}"
...
export EXPECTED_IMAGE WRONG_IMAGE EVENTS
...
grep -Fq 'non-empty merged web image was not generated in this checkout' "${TEST_ROOT}/confused stderr" ||
