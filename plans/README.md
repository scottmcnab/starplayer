...
| M1 S3M in the browser | **Landed** — follow-ups [B8](engine/complete/M1-task-B8-archive-zip.md) zip support landed, [B9](engine/M1-task-B9-output-and-mixer-options.md) output/mixer options in flight; owner listening check pending (`cargo xtask wasm && cargo xtask serve`). **Owner gate:** M2 does not start without the owner's confirmation | [M1](engine/M1-master-plan.md) | 7 task files ready, B1–B7. [B4](engine/complete/M1-task-B4-s3m-effects.md) is the accuracy core and the largest single task. Exit: **a real `.s3m` plays in a browser** |
