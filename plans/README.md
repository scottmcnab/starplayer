...
| M8 follow-up: DMA refill remediation | **Open — direct tone accepted clean; engine-tone image/configuration/cadence/loop accepted objectively 2026-09-14; 125 Hz and normal-music listening pending** — [I3a](engine/M8-task-I3a-dma-refill-remediation.md). The clean post-render 172.265625 Hz result accepts sample packing, transport, codec and analog output. The A1S `engine-tone` image crossed its allocation-safe native `B00` loop with stable counters and heap headroom while sending real-16-bit S3M through `EmbeddedPlayer<Linear>` | [M8](engine/M8-master-plan.md) | Six quanta, three equal 2 048-byte descriptors; engine cap 1/4, headphone analog −12 dB, routing and refill recovery unchanged |
