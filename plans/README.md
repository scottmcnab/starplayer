...
| M8 Embedded proof | **Landed 2026-09-11** — the A1S DMA transport soak passed 2026-09-14; owner headphone listening acceptance and remaining hardware checks are outstanding. All six tasks in `engine/complete/`: [I1](engine/complete/M8-task-I1-embedded-host-core.md) `starplayer-host-embedded` (byte-identical to the std host, goldens reproduced), [I2](engine/complete/M8-task-I2-flash-resident-modules.md) borrowed PCM and the `SPMI` module image, [I3](engine/complete/M8-task-I3-esp32-a1s-bringup.md) the `embedded/` workspace and ES8388/I2S bring-up, [I4](engine/complete/M8-task-I4-esp32-c5-riscv-bench.md) the ESP32-C5 bench and second bare-metal CI target, [I5](engine/complete/M8-task-I5-buttons-and-display.md) keys and the ST7789 (`lcd` feature, audio on core 1), [I6](engine/complete/M8-task-I6-web-control.md) captive portal, HTTP/WebSocket API and module upload. Every variant links; the descriptor-aligned A1S audio path has been flashed and soaked. **Owner**: complete [I3a](engine/M8-task-I3a-dma-refill-remediation.md)'s listening acceptance, then follow the remaining hardware steps in `embedded/README.md` | [M8](engine/M8-master-plan.md) | ESP32-A1S (Xtensa) through the ES8388; the C5 proves RISC-V; `embedded/` is its own workspace on `esp-1.97` |
| M8 follow-up: DMA refill remediation | **Open — 172 Hz descriptor-coherent tone accepted clean; engine and matched 125 Hz tones buzz; heap-backed constrained handoff awaits hardware retest and normal music** — [I3a](engine/M8-task-I3a-dma-refill-remediation.md). The matched recording's unwanted comb is spaced at the cadence of two DMA descriptors. Steady refill preserves one staged 2 048-byte descriptor through errors; its bytes are allocated only after player open and its immediate `push_with` closure copies and returns exactly one descriptor without rendering or batching | [M8](engine/M8-master-plan.md) | Six quanta, three equal 2 048-byte descriptors; engine cap 1/4, headphone analog −12 dB, routing and tone generators unchanged |
