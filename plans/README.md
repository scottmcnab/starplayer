...
| M4 Generalise to a synthesis engine | M4-lite **landed** 2026-09-03 ([E1](engine/complete/M4-task-E1-instrument-and-sample-model.md), [E2](engine/complete/M4-task-E2-linear-frequency-and-format-dialects.md), [E3](engine/complete/M4-task-E3-voice-lifecycle-and-trace-v2.md)); M4-full **in progress** — [E4](engine/complete/M4-task-E4-instrument-and-midi-source.md) landed the `Instrument` trait with `SampleInstrument` and `MappedInstrument`, the sixteen-channel `InstrumentRack`, the `EventFeed` trait, `ExternalEventQueue` and `MidiSource` with its own control tick; [E5](engine/complete/M4-task-E5-midi-codec-and-smf.md) codec and SMF, [E6](engine/complete/M4-task-E6-live-input.md) live input and [E7](engine/M4-task-E7-jam-mode.md) jam mode remain | [M4](engine/M4-master-plan.md) | Design goal 8 is cashed in: two real implementations, neither of them a tracker processor. MIDI-driven XM/IT articulation stays M11's, and architecture Q4 stays M10's |
| M5 XM support | **In progress** — [F1](engine/complete/M5-task-F1-xm-loader.md) landed the loader, [F2](engine/complete/M5-task-F2-xm-playback.md) landed playback (FastTracker 2's effect processor, envelopes, key-off and fadeout, auto-vibrato, linear and Amiga periods, the volume column, and the whole XM half of the conformance corpus) and [F5](engine/complete/M5-task-F5-xm-conformance-repairs.md) settled its twenty records — **87 of 93 cases pass**, two are accepted deviations and four are known failures, with accuracy-policy D42–D47 and D75–D79 and four XM `QuirkSet` fields. F2 absorbed F4's wiring, so F3 is what remains per the [concurrency plan](engine/M3-M6-concurrency-plan.md) | [M5](engine/M5-master-plan.md) | Envelopes, key-off/fadeout, linear frequency |
| M6 IT support | **In progress** — [G1](engine/complete/M6-task-G1-it-loader.md) the IT loader landed; [G3](engine/complete/M6-task-G3-it-playback.md) landed IT playback: `ItProcessor` with per-voice articulation, New Note Actions, the duplicate check and OpenMPT's voice-stealing rule (architecture **Q3**, now settled), the effect set and the volume column, `TempoModelId::ItModern` filled in, `ItLoopDialect`, and 121 pinned IT corpus cases wired. [G6](engine/complete/M6-task-G6-it-conformance-repairs.md) is the repair pass — **55 of 121 cases pass** (up from 39), with accuracy-policy D80–D86 and `ItLoopDialect::ModPlug116`; the 66 that remain are `G6-IT-001`…`007` in `conformance/known-failures.md`. G2 (filter) is concurrent; G4 and G5 follow per the [concurrency plan](engine/M3-M6-concurrency-plan.md) | [M6](engine/M6-master-plan.md) | NNA/DCT/DCA, voice stealing, resonant filter. The hardest format |
