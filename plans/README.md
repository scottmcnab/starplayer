...
| M8 follow-up: DMA refill remediation | **Open — 172 Hz direct tone accepted clean; 125 Hz engine tone buzzes; matched post-render A/B and normal-music listening pending** — [I3a](engine/M8-task-I3a-dma-refill-remediation.md). The engine-tone image crossed its allocation-safe native `B00` loop with stable counters, while its host render is smooth. The default-off `matched-tone` image renders the same engine work underneath a continuous integer-Q15 125 Hz override matched to its 4 796/5 327 channel peaks | [M8](engine/M8-master-plan.md) | Six quanta, three equal 2 048-byte descriptors; engine cap 1/4, headphone analog −12 dB, routing and refill recovery unchanged |
