...
| M5 XM support | **In progress** — [F1](engine/complete/M5-task-F1-xm-loader.md) landed the loader and [F2](engine/M5-task-F2-xm-playback.md) landed playback: FastTracker 2's effect processor, envelopes, key-off and fadeout, auto-vibrato, linear and Amiga periods, the volume column, and the whole XM half of the conformance corpus — 72 of 93 cases pass, with accuracy-policy D42–D47 and twelve `F2-XM-*` records. F2 absorbed F4's wiring, so F3 and F5 are what remain per the [concurrency plan](engine/M3-M6-concurrency-plan.md) | [M5](engine/M5-master-plan.md) | Envelopes, key-off/fadeout, linear frequency |
