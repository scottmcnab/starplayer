...
| M8 Embedded proof | **Landed 2026-09-11** — owner hardware run outstanding. All six tasks in `engine/complete/`: [I1](engine/complete/M8-task-I1-embedded-host-core.md) `starplayer-host-embedded` (byte-identical to the std host, goldens reproduced), [I2](engine/complete/M8-task-I2-flash-resident-modules.md) borrowed PCM and the `SPMI` module image, [I3](engine/complete/M8-task-I3-esp32-a1s-bringup.md) the `embedded/` workspace and ES8388/I2S bring-up, [I4](engine/complete/M8-task-I4-esp32-c5-riscv-bench.md) the ESP32-C5 bench and second bare-metal CI target, [I5](engine/complete/M8-task-I5-buttons-and-display.md) keys and the ST7789 (`lcd` feature, audio on core 1), [I6](engine/complete/M8-task-I6-web-control.md) captive portal, HTTP/WebSocket API and module upload. Every variant links; nothing has been flashed. **Owner**: follow the step lists in `embedded/README.md` — flash the `bench` builds and paste the hashes and cycle counts into `plans/reference/embedded-budget.md`, then listen | [M8](engine/M8-master-plan.md) | ESP32-A1S (Xtensa) through the ES8388; the C5 proves RISC-V; `embedded/` is its own workspace on `esp-1.97` |
...
| A4 Cast to Google Home | **In progress** — N1 and N2 pulled 2026-09-11 (task files in `plans/apps/`); N3/N4 wait for the N1 findings | [A4](apps/A4-master-plan.md) | Play a module on a Nest speaker or group from the web player (custom Web Receiver running the wasm engine) or the CLI (rendered FLAC/WAV to the Default Media Receiver) |
| A4-N1 cast probe | **Built** — owner registration, hosting and the on-device findings outstanding | [N1](apps/A4-task-N1-cast-probe.md) | `apps/starplayer-cast-probe/`: a development Web Receiver + Chrome sender that reports whether a Nest speaker's runtime has `WebAssembly`, `AudioWorklet` and `SharedArrayBuffer`, and how fast the wasm host renders there — the evidence N3/N4 are gated on |
