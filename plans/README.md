...
| M1 S3M in the browser | **In progress** — B1, B2, B3, B5 landed; B6 in flight; B4 next. **Owner gate:** pause after B7 for the listening check; M2 does not start without the owner's confirmation | [M1](engine/M1-master-plan.md) | 7 task files ready, B1–B7. [B4](engine/M1-task-B4-s3m-effects.md) is the accuracy core and the largest single task. Exit: **a real `.s3m` plays in a browser** |
...
| Milestone | Status | Master plan | Notes |
|---|---|---|---|
| Web player | Not started | delivered by [M0-A4](engine/complete/M0-task-A4-audioworklet-spike.md) + [M1-B7](engine/M1-task-B7-web-player.md) | The primary UI and the first deliverable |
