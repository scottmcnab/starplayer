...
| M5 XM support | **In progress** — [F1](engine/M5-task-F1-xm-loader.md) the XM loader is written: `probe`/`load` over `ModuleReader`, `XmCell` and `PatternView`, `XmPatternData`, two fuzz targets and six seeds, and all 140 XMs in the pinned libxmp corpus load. F2 (instrument runtime), F3 (effects), F4 (wiring) and F5 follow per the [concurrency plan](engine/M3-M6-concurrency-plan.md) | [M5](engine/M5-master-plan.md) | Envelopes, key-off/fadeout, linear frequency |
