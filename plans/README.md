...
| M7 DSP graph | **In progress** — pulled 2026-09-05; [H1](engine/complete/M7-task-H1-channel-buses-and-insert-graph.md) buses and the `Insert` trait and [H2](engine/complete/M7-task-H2-dsp-primitives.md) the DSP primitives and [H5](engine/complete/M7-task-H5-cubic-and-sinc-interpolation.md) cubic/sinc interpolation and [H3](engine/complete/M7-task-H3-eq-delay-chorus.md) EQ/delay/chorus and [H4](engine/complete/M7-task-H4-reverb-and-compressor.md) reverb/compressor and [H7](engine/complete/M7-task-H7-host-wiring.md) host wiring (`Player`, the CLI's `--insert`/`--list-effects`, the wasm host and the web page's Effects panel) landed (the exit criterion is an offline test, reproduced through `Player` and reachable from the CLI and the browser); H6 SIMD remains; owner listening check outstanding | [M7](engine/M7-master-plan.md) | Per-channel inserts, master bus, SIMD |
