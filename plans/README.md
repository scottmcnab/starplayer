...
| M3 Native surfaces | **In progress** — [D1](engine/complete/M3-task-D1-song-timeline-and-loop-detection.md) song timeline and [D2](engine/complete/M3-task-D2-natural-end-versus-loop.md) natural end versus loop landed; [D3](engine/M3-task-D3-facade-format-dispatch.md) facade format dispatch is the next task and the one M5/M6 wait for; D4–D7 follow per the [concurrency plan](engine/M3-M6-concurrency-plan.md) | [M3](engine/M3-master-plan.md) | cpal host, CLI, offline renderer, telemetry split. The wasm retrofit behind the host trait is deferred (owner, 2026-09-03) |
| M4 Generalise to a synthesis engine | **Split** — M4-lite in progress: [E1](engine/complete/M4-task-E1-instrument-and-sample-model.md) instrument/sample model, [E2](engine/complete/M4-task-E2-linear-frequency-and-format-dialects.md) linear-frequency table and dialects, [E3](engine/M4-task-E3-voice-lifecycle-and-trace-v2.md) voice lifecycle and trace v2; M4-full (MIDI, `Instrument` trait) deferred | [M4](engine/M4-master-plan.md) | Shared machinery for XM/IT, no `Instrument` trait until the MIDI sample player exists (owner, 2026-09-03) |
| M5 XM support | Planned — task graph F1–F5 in the [concurrency plan](engine/M3-M6-concurrency-plan.md); starts when E1–E3 land | [M5](engine/M5-master-plan.md) | Envelopes, key-off/fadeout, linear frequency |
| M6 IT support | Planned — task graph G1–G6 in the [concurrency plan](engine/M3-M6-concurrency-plan.md); starts when E1–E3 land, concurrently with M5 | [M6](engine/M6-master-plan.md) | NNA/DCT/DCA, voice stealing, resonant filter. The hardest format |
