...
| Status | M4-lite **landed** 2026-09-03; M4-full **implemented** 2026-09-04 — owner hardware jam check pending |
...
6. **Hosts stamp events.** `ExternalEventQueue` carries absolute frames; `Player::send_event`
   stamps `source_frame + lead` (two quanta by default), as E6 corrected: sources dispatch
   on the musical clock and it pauses with transport. Late events dispatch at the current
   frame and count a warning.
...
instruments; a keyboard triggers notes from a loaded module's instruments; both at once
over a playing module, with deterministic ordering.
