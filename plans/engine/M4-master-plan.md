...
|---|---|---|---|
| [E4](M4-task-E4-instrument-and-midi-source.md) | **Landed 2026-09-04.** `Instrument` trait, `SampleInstrument`, `MappedInstrument`, `InstrumentRack`, the `EventFeed` trait, `ExternalEventQueue`, `MidiSource` (an `EventSource` over any event feed with the rack inside), the control tick, channel base, telemetry, an offline determinism test with scripted events | — | Opus |
