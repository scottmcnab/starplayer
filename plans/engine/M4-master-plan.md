...
| Status | M4-lite **landed** 2026-09-03; M4-full **planned** 2026-09-04 — tasks E4–E7 below, not yet dispatched |
...
|---|---|---|---|
| [E4](M4-task-E4-instrument-and-midi-source.md) | `Instrument` trait, `SampleInstrument`, `MappedInstrument`, `InstrumentRack`, `ExternalEventQueue`, `MidiSource` (an `EventSource` over any event feed with the rack inside), the control tick, channel base, telemetry, an offline determinism test with scripted events | — | Opus |
| [E5](M4-task-E5-midi-codec-and-smf.md) | `starplayer-midi`: the byte codec to and from `Event`, the SMF parser and tempo map, `SmfSequencer`; facade `midi`/`smf` arms; `.mid` in the CLI's `info`/`render`/`play` with `--instruments` | E4 | Sonnet |
